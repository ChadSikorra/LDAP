...
namespace Tests\Unit\FreeDSx\Ldap\Server\Storage;
...
final class FilterEvaluatorTest extends TestCase
{
    ...
    {
        $filter = new MatchingRuleFilter(null, 'cn', 'ALICE');
        self::assertTrue($this->subject->evaluate($this->entry, $filter));
    }

    public function test_matching_rule_case_exact_match(): void
    {
        $filter = new MatchingRuleFilter('2.5.13.5', 'cn', 'Alice');
        self::assertTrue($this->subject->evaluate($this->entry, $filter));
    }
    ...
    public function test_matching_rule_bit_and(): void
    {
        ...
        // 1001 & 8 = 8 => true
        $filter = new MatchingRuleFilter('1.2.840.113556.1.4.803', 'uidNumber', '8');
        self::assertTrue($this->subject->evaluate($this->entry, $filter));
    }
    ...
    {
        // 1001 & 2 = 0 => false
        $filter = new MatchingRuleFilter('1.2.840.113556.1.4.803', 'uidNumber', '2');
        self::assertFalse($this->subject->evaluate($this->entry, $filter));
    }

    public function test_matching_rule_unknown_returns_false(): void
    {
        $filter = new MatchingRuleFilter('1.2.3.4.5.unknown', 'cn', 'Alice');
        self::assertFalse($this->subject->evaluate($this->entry, $filter));
    }

    public function test_matching_rule_dn_attributes(): void
    {
        $filter = new MatchingRuleFilter(null, 'cn', 'alice', true);
    ...
    public function test_matching_rule_null_attribute_matches_all(): void
    {
        ...
        $filter = new MatchingRuleFilter(null, null, 'Smith');
        self::assertTrue($this->subject->evaluate($this->entry, $filter));
    }
    ...
    {
        // A custom filter that is not one of the known types
        $unknownFilter = new class implements \FreeDSx\Ldap\Search\Filter\FilterInterface {
            public function toAsn1(): \FreeDSx\Asn1\Type\AbstractType
            {
                throw new \LogicException('Not implemented');
            }

            public static function fromAsn1(\FreeDSx\Asn1\Type\AbstractType $type): static
            {
                throw new \LogicException('Not implemented');
            }
        ...
        self::assertFalse($this->subject->evaluate($this->entry, $unknownFilter));
    }
}
