...
final class FilterEvaluator implements FilterEvaluatorInterface
{
    ...
    private function evaluateAnd(Entry $entry, AndFilter $filter): bool
    {
    ...
    private function evaluateOr(Entry $entry, OrFilter $filter): bool
    {
        foreach ($filter->get() as $child) {
            if ($this->evaluate($entry, $child)) {
                return true;
    ...
    private function evaluateNot(Entry $entry, NotFilter $filter): bool
    {
        return !$this->evaluate($entry, $filter->get());
    }

    private function evaluatePresent(Entry $entry, PresentFilter $filter): bool
    {
        return $entry->has($filter->getAttribute());
    }

    private function evaluateEquality(Entry $entry, EqualityFilter $filter): bool
    {
    ...
    private function evaluateSubstring(Entry $entry, SubstringFilter $filter): bool
    {
        $attribute = $entry->get($filter->getAttribute());
    ...
    private function evaluateGreaterOrEqual(Entry $entry, GreaterThanOrEqualFilter $filter): bool
    {
        $attribute = $entry->get($filter->getAttribute());
    ...
    private function evaluateLessOrEqual(Entry $entry, LessThanOrEqualFilter $filter): bool
    {
        $attribute = $entry->get($filter->getAttribute());
    ...
    /**
     * Compare two values for ordering. Uses numeric comparison when both are numeric,
     * otherwise falls back to case-insensitive string comparison.
     */
    private function compareOrdered(string $value, string $filterValue): int
    {
        if (is_numeric($value) && is_numeric($filterValue)) {
    ...
    private function evaluateApproximate(Entry $entry, ApproximateFilter $filter): bool
    {
    ...
    private function evaluateMatchingRule(Entry $entry, MatchingRuleFilter $filter): bool
    {
        ...
        if ($filterAttributeName !== null) {
            ...
            if ($attribute !== null) {
                $valuesToTest = $attribute->getValues();
            }
        } else {
        ...
        if ($filter->getUseDnAttributes()) {
            foreach ($entry->getDn()->toArray() as $rdn) {
                $valuesToTest[] = $rdn->getValue();
            }
        }

        foreach ($valuesToTest as $value) {
            if ($this->matchByRule($matchingRule, $value, $filterValue)) {
                return true;
            }
        }
    ...
    private function matchByRule(?string $rule, string $value, string $filterValue): bool
    {
        return match ($rule) {
